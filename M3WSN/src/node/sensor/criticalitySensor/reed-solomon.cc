...
// todo:comentei estes typedef e coloquei no .h
...
static char *allPp[] = {    /* GF_BITS	polynomial		*/
    ...
    (char*)"111",		    /*  2	1+x+x^2			*/
    (char*)"1101",		    /*  3	1+x+x^3			*/
    (char*)"11001",		    /*  4	1+x+x^4			*/
    (char*)"101001",		    /*  5	1+x^2+x^5		*/
    (char*)"1100001",		    /*  6	1+x+x^6			*/
    (char*)"10010001",		    /*  7	1 + x^3 + x^7		*/
    (char*)"101110001",	    /*  8	1+x^2+x^3+x^4+x^8	*/
    (char*)"1000100001",	    /*  9	1+x^4+x^9		*/
    (char*)"10010000001",	    /* 10	1+x^3+x^10		*/
    (char*)"101000000001",	    /* 11	1+x^2+x^11		*/
    (char*)"1100101000001",	    /* 12	1+x+x^4+x^6+x^12	*/
    (char*)"11011000000001",	    /* 13	1+x+x^3+x^4+x^13	*/
    (char*)"110000100010001",	    /* 14	1+x+x^6+x^10+x^14	*/
    (char*)"1100000000000001",	    /* 15	1+x+x^15		*/
    (char*)"11010000000010001"	    /* 16	1+x+x^3+x^12+x^16	*/
};
...
#define NEW_GF_MATRIX(rows, cols) \
    (gf *)my_malloc(rows * cols * sizeof(gf), (char*)" ## __LINE__ ## " )
...
static int invert_mat(gf *src, int k) {
    ...
    int error = 1 ;
    int *indxc = (int*)my_malloc(k*sizeof(int), (char*)"indxc");
    int *indxr = (int*)my_malloc(k*sizeof(int), (char*)"indxr");
    int *ipiv = (int*)my_malloc(k*sizeof(int), (char*)"ipiv");
...
void fec_free(struct fec_parms *p) {
                     // modifiquei o (int)(p->enc_matrix) por (u_long)
    if (p==NULL ||    // erro: cast from ‘ns3::gf*’ to ‘int’ loses precision
...
struct fec_parms * fec_new(int k, int n) {
    ...
    retval = (fec_parms*)my_malloc(sizeof(struct fec_parms), (char*)"new_code");
    retval->k = k ;
    retval->n = n ;
    retval->enc_matrix = NEW_GF_MATRIX(n, k);
    					// modifiquei o (int)(retval->enc_matrix) por (u_long)
    					// erro: cast from ‘ns3::gf*’ to ‘int’ loses precision
...
static gf * build_decode_matrix(struct fec_parms *code, gf *pkt[], int index[]) {
	...
	if (index[i] < code->n )
	    ...
	    //fprintf(stderr, "decode: invalid index[%i] = %d (k %d) (n %d)\n", i, index[i], code->k, code->n - 1 );
	    free(matrix) ;
	    return NULL ;
	}
    }
...
int fec_decode(struct fec_parms *code, gf *pkt[], int index[], int sz) {
	...
	//fprintf(stderr, "shuffle: invalid (k %d)\n", k );
	return 1 ;
    }
     ...
     */
    new_pkt = (gf**)my_malloc (k * sizeof (gf * ), (char*)"new pkt pointers" );
    for (row = 0 ; row < k ; row++ ) {
	if (index[row] >= k) {
	    new_pkt[row] = (gf*)my_malloc (sz * sizeof (gf), (char*)"new pkt buffer" );
...
struct fec_parms* ReedSolomonFec::create(int k, int n) {
	return fec_new(k,n);
}

void ReedSolomonFec::destroy(struct fec_parms* code) {
	fec_free(code);
}

void ReedSolomonFec::encode(void *code, void *src[], void *dst, int index, int sz) {
	fec_encode((struct fec_parms*)code, (gf**)src, (gf*)dst, index, sz);
	return;
}

int ReedSolomonFec::decode(void *code, void *pkt[], int index[], int sz) {
	return fec_decode((fec_parms*)code, (gf**)pkt, index, sz);
}

void * ReedSolomonFec::safeMalloc(int sz, const char *s) {
...
void ReedSolomonFec::BuildSampleData(u_char **frame, int k, int sz) {
	for (int i = 0 ; i < k ; i++ ) {
		for (int item=0; item < sz; item++)
			frame[i][item] = ((item ^ i) + 3) & 0xFF;
	}
}
