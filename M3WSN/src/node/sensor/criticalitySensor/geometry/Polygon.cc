...
Polygon::iterator Polygon::remove(const iterator & target) {
    if (!(vertices.empty())) {
        std::list<Point>::iterator ret_value;
...
bool Polygon::contains(const Point & p) const {
    ...
    do {
        ...
        if ((*current).is_on(line)) {
            ...
            if (turn(line.get_p1(), line.get_p2(), v) == turn(line.get_p1(), line.get_p2(), *current)) {
                intersections--;
            }
...
Polygon Polygon::convex_hull() const {
    ...
    do {
        ...
        if (turn(*(first + 1), *first, p) == left_turn || turn(*(last - 1), *last, p) == right_turn) {
            hull.push_back(p);
            hull.push_front(p);
