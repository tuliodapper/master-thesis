...
#include "VideoSensorFEC.h"
#include "Segment.h"
#include "WiseBaseApplication.h"
#include "WiseMovingTargetMessage_m.h"

Define_Module(VideoSensorFEC);

VideoSensorFEC::VideoSensorFEC() {
 ...
 */
void VideoSensorFEC::computeCoverSets() {
 ...
 *
 */
void VideoSensorFEC::initialize() {
 ...
 *
 */
std::set<unsigned int> VideoSensorFEC::computeIntersection(const std::set<unsigned int>& A, const std::set<unsigned int>& B) const {
 ...
 */
void VideoSensorFEC::handleSample(cMessage* msg) {
...
void VideoSensorFEC::update_location(){
	if (changeAngle ){
    		int intersections = 0;
	    	lineOfSight = 0;
	    	Segment direction[2] = {
...
void VideoSensorFEC::handleSensorMessage(cMessage* msg) {
	if (msg->getKind() == 10){
...
void VideoSensorFEC::getNextFrame(unsigned int& ndx, struct tracerec& t) {
    if (uid != 0)
...
void VideoSensorFEC::processStreamData() {
...
void VideoSensorFEC::SendWithoutFEC (int nbytes, int k){
...
void VideoSensorFEC::SendFrameWithFEC(unsigned int fullPkt, unsigned int restPkt) {
...
void VideoSensorFEC::BuildFecBlockAndSend(unsigned int _k, unsigned int _n, unsigned int _sz) {
	// build and send
...
void VideoSensorFEC::Send (WiseSensorMessage *pkt, bool generateTrace, int _k) {
	if (generateTrace){
...
void VideoSensorFEC::contLTrace(int cLin){
     nLin = cLin;
}

int VideoSensorFEC::getContLTrace(){
    return nLin;
}

void VideoSensorFEC::change_angle(){
    int intersections = 0;
    lineOfSight = 0;
 ...
 */
std::string VideoSensorFEC::keyFromSet(const std::set<unsigned int>& set) const {
    std::stringstream ss;
 ...
 *
 */
bool VideoSensorFEC::covers(double x, double y) {
