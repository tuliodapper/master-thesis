...
//*										*
//*  	Module: Sensor application to sensor node				*
//*										*
//*  	Version: 1.0								*
...
#include <map>
#include <algorithm>
#include "OmnidirectionalSensor.h"
#include "Segment.h"
#include "WiseBaseApplication.h"
#include "WiseMovingTargetMessage_m.h"
#include "WisePhysicalProcessMessage_m.h"

Define_Module(OmnidirectionalSensor);

OmnidirectionalSensor::OmnidirectionalSensor() {
 ...
 */
void OmnidirectionalSensor::computeCoverSets() {
 ...
 *
 */
void OmnidirectionalSensor::initialize() {
 ...
 *
 */
void OmnidirectionalSensor::finish() {
    CritSensor::finish();
}
 ...
 *
 */
std::set<unsigned int> OmnidirectionalSensor::computeIntersection(const std::set<unsigned int>& A, const std::set<unsigned int>& B) const {
 ...
 *
 */
void OmnidirectionalSensor::handleSample(cMessage* msg) {
...
void OmnidirectionalSensor::update_location(){
 ...
 *
 */
void OmnidirectionalSensor::draw_node() {
    NodeLocation location = mobilityModule->getLocation();
    p.x = location.x;
    p.y = location.y;
    caminfo.set_position(location.x, location.y, 0);
 ...
 */
std::string OmnidirectionalSensor::keyFromSet(const std::set<unsigned int>& set) const {
    std::stringstream ss;
 ...
 *
 */
bool OmnidirectionalSensor::covers(double x, double y) {
    return myFoV.contains(Point(x, y));
}

void OmnidirectionalSensor::handleSensorMessage(cMessage* msg) {
