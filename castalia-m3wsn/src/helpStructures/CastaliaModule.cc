...
string getSensorIdForTest(string str){
    string result = "";

    // char str[]="SN.node[17].MobilityManager";
    for (int i = 0; i < 12; ++i)
    {
        if (isdigit(str[i]))
        {
...
void CastaliaModule::finish() {
	...
	for (i1 = simpleoutputs.begin(); i1 != simpleoutputs.end(); i1++) {
		...
		for (i2 = outByIndex->byIndex.begin(); i2 != outByIndex->byIndex.end(); i2++) {
			...
			if (header) {
				...
				result() << CASTALIA_PREFIX << "module:" << getFullPath() << endl;
				header = false;
			}
			EV << CASTALIA_PREFIX << "\t";
			result() << CASTALIA_PREFIX << "\t";
			if (index >= 0){
				EV << " index:" << index << " ";
				result() << " index:" << index << " ";
			}
			EV << "simple output name:" << descr << endl;
			result() << "simple output name:" << descr << endl;
			map<string,double>::iterator i3;
			for (i3 = out->data.begin(); i3 != out->data.end(); i3++){
				EV << CASTALIA_PREFIX "\t\t" << i3->second << " " << i3->first << endl;
				result() << CASTALIA_PREFIX "\t\t" << i3->second << " " << i3->first << endl;
			}
		}
	}
	simpleoutputs.clear();
	...
	for (i4 = histograms.begin(); i4 != histograms.end(); i4++) {
		...
		for (i5 = hist->byIndex.begin(); i5 != hist->byIndex.end(); i5++) {
			...
			result() << "histogram name:" << descr << endl;
			EV << CASTALIA_PREFIX << "\thistogram_min:" << hist->min << " histogram_max:" << hist->max << endl;
			result() << CASTALIA_PREFIX << "\thistogram_min:" << hist->min << " histogram_max:" << hist->max << endl;
			EV << CASTALIA_PREFIX << "\thistogram_values";
			result() << CASTALIA_PREFIX << "\thistogram_values";
			for (int i = 0; i <= hist->numBuckets; i++){
				EV << " " << histBuckets->buckets[i];
				result() << " " << histBuckets->buckets[i];
			}
			EV << endl;
			result() << endl;
		}
...
std::ostream & CastaliaModule::result() {
	return (ostream &) ResultInfoWriter::getStream();
}

std::ostream & CastaliaModule::trace() {
	string sensorId = getSensorIdForTest(getFullPath());
...
std::ostream & CastaliaModule::output() {
	if (hasPar("collectOutputInfo") && par("collectOutputInfo")) {
		return (ostream &) OutputInfoWriter::getStream() << "\n";
	} else {
		return empty;
	}
}
...
void CastaliaModule::collectOutputNocheck(const char *descr, int index,	const char *label, double amt){
...
void CastaliaModule::declareHistogram(const char *descr, double min, double max, int buckets){
...
void CastaliaModule::powerDrawn(double power) {
		...
		if (name.compare("Radio") == 0) 
			...
			classPointers.resourceManager = getParentModule()->getSubmodule("ResourceManager");
	    		...
	    		classPointers.resourceManager = getParentModule()->getParentModule()->getSubmodule("ResourceManager");
		else if (name.compare("MobilityManager") == 0)
	    		classPointers.resourceManager = getParentModule()->getSubmodule("ResourceManager");
		else
