...
void VirtualMobilityManager::handleMessage(cMessage * msg){
...
void VirtualMobilityManager::setLocation(NodeLocation newLocation){
	nodeLocation = newLocation;
	notifyWirelessChannel();
}

void VirtualMobilityManager::notifyWirelessChannel(){
	//trace() << "notifyWirelessChannel";
	if (nodeLocation.x > 0 && nodeLocation.y > 0){
		...
		trace() << "erro " << nodeLocation.x << " " << nodeLocation.y << " " << nodeLocation.z;
}

NodeLocation VirtualMobilityManager::getLocation(){
	return nodeLocation;
}

double VirtualMobilityManager::getEnergyToFly(){
	return energyToFly;
}

double VirtualMobilityManager::getSpeed(){
	return speed;
}

int VirtualMobilityManager::getMovingTrajectory(){
	return movingTrajectory;
}

double VirtualMobilityManager::getStartTime_Final(){
    return startTime_final;
}

double VirtualMobilityManager::getEndTime_Final(){
    return endTime_final;
}

double VirtualMobilityManager::getX_Final(){
    return x_final;
}

double VirtualMobilityManager::getY_Final(){
    return y_final;
}

double VirtualMobilityManager::getMaxSpeed(){
	return maxSpeed;
}

double VirtualMobilityManager::getMovimentRange(){
    return rayon;
}

void VirtualMobilityManager::toApplicationLayer(cMessage * msg) {
	send(msg, "toApplicationModule");
}

void VirtualMobilityManager::fromApplicationLayer(cMessage * msg) {
	trace() << "fomApplicationLayer";
}

double VirtualMobilityManager::getWaypointX(){
    return .0;
}
double VirtualMobilityManager::getWaypointY(){
    return .0;
}
double VirtualMobilityManager::getWaypointZ(){
    return .0;
}
double VirtualMobilityManager::getEndTime(){
    return .0;
}
