		...
		case WC_SIGNAL_END:{
			...
			if (endingSignal->bitErrors != ALL_ERRORS) {
				if (endingSignal->bitErrors <= maxErrorsAllowed(endingSignal->encoding)) {
					...
					trace() << "power_dBm " << endingSignal->power_dBm;
					trace() << "maxInterference " << endingSignal->maxInterference;
					trace() << "LQI Ack: " << macPkt->getMacRadioInfoExchange().LQI << "\n";
					...
					// collect stats
					trace() << "LQI Perda: " << endingSignal->power_dBm - endingSignal->maxInterference << "\n";
					if (endingSignal->maxInterference == RXmode->noiseFloor) {
						stats.RxFailedNoInterference++;
						trace() << "endingSignal->maxInterference " << endingSignal->maxInterference;
						trace() << "RXmode->noiseFloor " << RXmode->noiseFloor;
...
double Radio::SNR2BER(double SNR_dB){
	switch (RXmode->modulation) {
		...
		case DIFFBPSK:{
			trace() << "DENIS - RXmode->noiseBandwidth " << RXmode->noiseBandwidth;
			trace() << "RXmode->datarate " << RXmode->datarate;
			trace() << "RXmode->datarate " << RXmode->datarate / 1000.0;
			trace() << "noiseBandwidth / datarate " << RXmode->noiseBandwidth / RXmode->datarate;
			trace() << "SNR " << SNR_dB;
			trace() << "pow SNR " << pow(10.0, (SNR_dB / 10.0));
			trace() << "0.5 * exp((" << RXmode->noiseBandwidth << "/" << RXmode->datarate << ") * " << pow(10.0, (SNR_dB / 10.0)) << ") = " << 0.5 * exp((RXmode->noiseBandwidth / RXmode->datarate) * pow(10.0, (SNR_dB / 10.0)));
...
void Radio::readIniFileParameters(void)
{
	...
	trace() << "startingMode " << startingMode;
	trace() << "Initialized RX mode to " << RXmode->name;
	trace() << "Modulation " << RXmode->modulation;
	...
	while (getline(f, s)) {
		...
		if (s.compare("RX MODES") == 0) {
			...
			if (section == 1) {
				...
				trace() << "datarate " << rxmode.datarate << "bps";
				ct = t.nextToken();
...
double Radio::getDataRate(){
	return RXmode->datarate;
}

int Radio::getPhyFrameOverhead(){
	return PhyFrameOverhead;
}

double Radio::getNoiseFloor(){
	return RXmode->noiseFloor;
}
