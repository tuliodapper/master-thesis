...
int VirtualRouting::bufferPacket(cPacket * rcvFrame)
{
	if ((int)TXBuffer.size() >= netBufferSize) {
        ...
        newOutput("drp;net");
        trace() << "Packet DROPPED at NET layer!";
		return 0;
	} else {
		TXBuffer.push(rcvFrame);
        trace() << "Packet buffered at NET layer. Buffer: " << TXBuffer.size() << "/" << netBufferSize;
		return 1;
	}
}
...
void VirtualRouting::newOutput(std::ostream& ss){
	newOutput(dynamic_cast<std::stringstream&>(ss).str());
}

void VirtualRouting::newTrace(std::ostream& ss){
	newTrace(dynamic_cast<std::stringstream&>(ss).str());
}

void VirtualRouting::newOutput(std::string str){
	output() << simTime() << ";" << self << ";" << str;
}

void VirtualRouting::newTrace(std::string str){
	stringstream ss;
	ss << simTime() << "\t" << self << "\t" << str;
