...
void WirelessChannel::initialize(int stage)
{
	...
	if (onlyStaticNodes) {
		...
		trace() << "numOfYCells " << numOfYCells << " | yCellSize " << yCellSize;
	 ...
	 **************************************************************/
	nodeLocation = new NodeLocation[numOfNodes];
	...
	topo->extractByNedTypeName(cStringTokenizer("wise_mnet.node.Node").asVector());
...
void WirelessChannel::handleMessage(cMessage * msg)
{
	...
	case WC_NODE_MOVEMENT:{
	     ...
	     *****************************************************/
		trace() << "WC_NODE_MOVEMENT";
			if (onlyStaticNodes){
				opp_error("Error: Rerceived WS_NODE_MOVEMENT msg, while onlyStaticNodes is TRUE");
				trace() << "Error: Rerceived WS_NODE_MOVEMENT msg, while onlyStaticNodes is TRUE";
			}
			...
			double testex = mobilityMsg->getX();
			double testey = mobilityMsg->getY();
			double testez = mobilityMsg->getZ();
			if ((nodeLocation[srcAddr].x < 0.0) || (nodeLocation[srcAddr].y < 0.0) || (nodeLocation[srcAddr].z < 0.0)){
				...
				trace() << "Wireless channel received faulty WC_NODE_MOVEMENT msg. We cannot have negative node coordinates ";
			}
			...
			if (((xIndex - 1) * xCellSize) >= nodeLocation[srcAddr].x)
				...
				if (nodeLocation[srcAddr].x > xFieldSize){
					...
					trace() << "WARNING at WC_NODE_MOVEMENT: node position out of bounds in X dimension!\n";
				}
			}
			...
			if (((yIndex - 1) * yCellSize) >= nodeLocation[srcAddr].y)
				...
				if (nodeLocation[srcAddr].y > yFieldSize){
					...
					trace() << "WARNING at WC_NODE_MOVEMENT: node position out of bounds in Y dimension!\n";
				}
			}
			...
			if (((zIndex - 1) * zCellSize) >= nodeLocation[srcAddr].z)
				...
				if (nodeLocation[srcAddr].z > zFieldSize){
					...
					trace() << "WARNING at WC_NODE_MOVEMENT: node position out of bounds in Z dimension!\n";
				}
			}

			int newCell = zIndex * zIndexIncrement + yIndex * yIndexIncrement + xIndex * xIndexIncrement;
			if (newCell != oldCell) {
				trace() << "newCell != oldCell";
...
void WirelessChannel::readIniFileParameters(void){
	...
	OutputInfoWriter::setOutputFileName(getParentModule()->par("outputInfoFileName").stringValue());
	ResultInfoWriter::setResultFileName(getParentModule()->par("resultInfoFileName").stringValue());
	ResultInfoWriter::setConfig(par("configName").stringValue());
	ResultInfoWriter::start();
...
double WirelessChannel::getD0(){
	return d0;
}

double WirelessChannel::getPLd0(){
	return PLd0;
}

double WirelessChannel::getPathLossExponent(){
