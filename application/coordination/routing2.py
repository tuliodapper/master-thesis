...
class Routing(object):
	...
	def __init__(self, config):
		...
		config["weigth_transmission"] = 100
		self.config 	= config
	...
	def initialize(self):
		self.graph 		= Graph()
		for i1 in self.config["namesUN"]:
			id1 = self.config["namesUN"].index(i1)
			min_distance = None
			for i2 in self.config["namesRN"]:
				id2 = self.config["namesRN"].index(i2)
				distance = utils.computeDistance(self.config["posUN"][id1,:], self.config["posRN"][id2,:])
				if ((min_distance is None) or \
					(distance < min_distance)):
					min_distance = distance
					id2_min = id2
			self.graph.add_edge(i1, self.config["namesRN"][id2_min], {'cost': distance})
			self.graph.add_edge(self.config["namesRN"][id2_min], i1, {'cost': distance})

		for i1 in self.config["namesRN"]:
			id1 = self.config["namesRN"].index(i1)
			for i2 in self.config["namesRN"]:
				if (i1 != i2):
					id2 = self.config["namesRN"].index(i2)
					distance = utils.computeDistance(self.config["posRN"][id1,:], self.config["posRN"][id2,:])
					if ((self.config["dcm"] is not None) and \
						(distance > self.config["dcm"])):
						distance = distance**2
					self.graph.add_edge(i1, i2, {'cost': distance})
					self.graph.add_edge(i2, i1, {'cost': distance})

	def updateTransmission(self, relayId, sign):
		for src in self.graph._data:
			for dst in self.graph._data[src]:
				if 	(str(src) == str(relayId)) or \
					(str(dst) == str(relayId)):
					self.graph._data[src][dst]['cost'] = self.graph._data[src][dst]['cost'] + sign*self.config["weigth_transmission"]

	def addTransmission(self, relayId):
		self.updateTransmission(relayId, 1)
		print("ADD TRANSMISSION -> relayId: " + str(relayId))

	def deleleTransmission(self, relayId):
		self.updateTransmission(relayId, -1)
		print("DELELE TRANSMISSION -> relayId: " + str(relayId))

	def computeRoute(self, nodeId, destination):
		try:
			path = find_path(self.graph, str(nodeId), str(destination), cost_func=self.cost_func)
			if 	(len(path.nodes) > 0) :
				route = {	"nodeId": 		str(nodeId),
							"nextHop": 		str(path.nodes[1]),
							"destination": 	str(destination)}
		except Exception as e:
			route = None
		return route

	def computeRoutes(self, nodeId, destination):
		routes = []
		try:
			path = find_path(self.graph, str(nodeId), str(destination), cost_func=self.cost_func)
			n = len(path.nodes)
			for i in range(n):
				if (i < (n-1)):
					routes.append({	"nodeId": 		str(path.nodes[i]),
									"nextHop": 		str(path.nodes[i+1]),
									"destination": 	str(path.nodes[-1])})
		except Exception as e:
			routes = None
		return routes
		...
		for i2 in range(i1, nMU):
			if (i1 != i2):
				try:
					...
					if 	(len(path.nodes) > 0) and \
						(len(set(path.nodes[1:(len(path.nodes)-1)]).intersection(namesNodesMU)) == 0):
...
def cleanRoutes(namesNodesMU, posNodesMU, namesNodesRU, posNodesRU, routes, dcm):
	...
	for route in range(len(routes)):
		...
		for iNode in range(len(routes[iRoute])-1):
			...
			distance 	= utils.computeDistance(posNode1, posNode2)
			if distance > dcm:
				printAndSave(	"delete route =>  F: " + nameNodeFrom + \
								" - T: " + nameNodeTo + \
								" | D: " + str(distance) + \
								" - F: " + nameNode1 + \
								" - T: " + nameNode2)
...
def processRoute(routes):
	...
	for route in routes:
		...
		for iNode in range(n):
			if (iNode < (n-1)):
				routes2.append({"nodeId": 		str(route[iNode]),
								"nextHop": 		str(route[iNode+1]),
								"destination": 	str(route[-1])})
			if (iNode > 0):
				routes2.append({"nodeId": 		str(route[iNode]),
								"nextHop": 		str(route[iNode-1]),
